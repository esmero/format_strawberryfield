...
BookReader.prototype.urlStartLocationPolling =  (function(super_) {
  return function (options) {
    //super_.call(this, options);
  ...
  const updateHash = () => {
    // This is different now. Only read from hash. Forget History
    const newFragment = this.urlReadHashFragment();
...
})(BookReader.prototype.urlStartLocationPolling);
...
BookReader.prototype.parseSequence = function (sequenceId) {
  ...
  if(self.getApiVersion() == "3.x") {
    ...
    // April 2024. Bc we are loading manifest async now, the Fragment listener *url plugin * code will
    // lose context (this) and will reset the fragment to the last remembered one
    // So we have the chance here to fill the gap but tapping into it, loading the fragment and setting it
    // into the defaults (which override that URL)
    // Only way we can allow a manifest to lead on the display mode + page order + a passed search.
    self.mode = processBehaviorToIABookreaderModeV3(self.jsonLd);
    const fragment_params = Drupal.FormatStrawberryfieldIiifUtils.getUrlFragments()
    self.defaults = "mode/" + self.mode + 'up';
    if (typeof(fragment_params.search) != "undefined") {
      self.defaults = self.defaults + "/search/" + fragment_params.search;
      self.searchTerm = fragment_params.search;
    }
    if (self.defaults) {
      if (!self.suppressFragmentChange) {
    ...
    if (top_level_behavior != 0) {
      if (sequence_level_behavior == 0) {
        self.mode = top_level_behavior;
      }
      else {
        self.mode = sequence_level_behavior;
      }
    }
    ...
    if (typeof(self.mode) !== "undefined" && self.mode) {
      self.defaults = "mode/" + self.mode + 'up';
    }
    const fragment_params = Drupal.FormatStrawberryfieldIiifUtils.getUrlFragments()
    if (typeof(fragment_params.search) != "undefined") {
...
BookReader.prototype.search =  (function(super_) {
  ...
  return function(term = '', overrides = {}) {
    ...
    if (this.bookPath.length - this.bookPath.lastIndexOf(subPrefixWithSlash) == subPrefixWithSlash.length) {
      path = this.bookPath.substring(0, this.bookPath.length - subPrefixWithSlash.length);
    }
