...
class LeaftletGeoJSON extends StylePluginBase {
  ...
  protected function defineOptions() {
    ...
    $options['metadatadisplayentity_uuid'] = ['default' => NULL];
    ...
    $options['json_key_source'] =  ['default' => ''];
  ...
  public function buildOptionsForm(&$form, FormStateInterface $form_state) {
    ...
    $all_options_form_source = [
      'metadatadisplayentity' => $this->t(
        'GeoJSON generated by an Metadata Display Entity'
      ),
    ...
    $form['json_key_source'] = [
      '#type'          => 'textfield',
      '#title'         => t(
        'JSON Key or JMESPath search string that needs to exist and not be empty to render a map.'
      ),
      '#description'   => t(
        'Leave empty to render always a map. If present and value is empty, Map will not be rendered'
      ),
      '#default_value' => trim($this->options['json_key_source']),
    ];
    $form['tilemap_url'] = [
      '#type'          => 'textfield',
    ...
    $form['mediasource'] = [
      ...
      '#default_value' => $this->options['mediasource'],
      '#required'      => TRUE,
      '#attributes'    => [
    ...
    $form['metadatadisplayentity_uuid'] = [
      ...
      '#description' => $this->t(
        'This template will be used for Metadata Exposed Entities and also for Node Lists as Processing source for GeoJSON'
      ),
      '#selection_handler' => 'default:metadatadisplay',
      '#validate_reference' => TRUE,
      '#required' => TRUE,
      '#default_value' => $metadata_display_entity,
    ];
    ...
    $form['geojsonurl_json_key_source'] = [
      '#type'          => 'textfield',
      '#title'         => t(
        'JSON Key from where to fetch one or more GeoJSON URLs. URLs can be external.'
      ),
      '#default_value' => $this->options['geojsonurl_json_key_source'],
      '#states'        => [
        'visible' => [
    ...
    $form['geojsonnodelist_json_key_source'] = [
      '#type'          => 'textfield',
      '#title'         => t(
      ...
      '#default_value' => $this->options['geojsonnodelist_json_key_source'],
      '#states'        => [
        'visible' => [
          ':input[data-formatter-selector="mediasource"][value="geojsonnodelist"]' => ['checked' => TRUE],
        ],
    ...
    if (empty($options_for_mainsource)) {
      ...
      $form['main_mediasource']['#empty_option'] = t(
  ...
  public function processGeoJsonforMetadataExposeEntity(
    ...
    if ($this->options['metadataexposeentity_source'] &&
      !empty($this->options['metadataexposeentity_source'])) {
  ...
  public function processGeoJsonforURL(
    ...
    if ($this->options['geojsonurl_json_key_source'
    ]) {
      $jsonkey = $this->options['geojsonurl_json_key_source'];
  ...
  public function processGeoJsonforNodeList(
    ...
    $geojsons = [];
    $cleannodelist = [];
    if ($this->options['geojsonnodelist_json_key_source'] && $this->options['metadataexposeentity_source']) {
      $jsonkey = $this->options['geojsonnodelist_json_key_source'];
      $entity = $this->entityTypeManager->getStorage(
        'metadataexpose_entity'
      )->load($this->options['metadataexposeentity_source']);
      if ($entity) {
